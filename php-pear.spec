...
Obsoletes: %{?scl_prefix}php-pear-XML-Util < %{xmlutil}-%{release}
Provides:  %{?scl_prefix}php-pear-XML-Util = %{xmlutil}-%{release}
...
sed -e 's/@SCL@/%{?scl:%{scl}_}/' \
    ...
    -e 's:@BINDIR@:%{_bindir}:' \
    -i $RPM_BUILD_ROOT%{_root_sysconfdir}/rpm/macros.%{name}
cat $RPM_BUILD_ROOT%{_root_sysconfdir}/rpm/macros.%{name}
